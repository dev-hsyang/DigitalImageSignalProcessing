...
template<typename T> T at(const Mat& m, int y, int x){
    /*
    x = min(m.cols-1, max(0,x));
    y = min(m.rows-1, max(0,y)); //replicate padding
    */
    
    //constant padding :: zero padding
    if(x<0) return T(0);
    if(x>m.cols-1) return 0;
    if(y<0) return T(0);
    if(y>m.rows-1) return 0;
    return m.at<T>(y,x);
}
...
void imshowF(const String& windowName, const Mat& m){
    Mat mag;
    vector<Mat> channels;
    split(m, channels);
    magnitude(channels[0], channels[1], mag);
    log(mag+1, mag);
    Mat out(mag.size(), mag.type());
    mag(Rect(0, 0, mag.cols/2, mag.rows/2)).copyTo(out(Rect(mag.cols/2, mag.rows/2, mag.cols/2, mag.rows/2)));
    imshow(windowName, mag/3.f);
}

int main(int argc, const char * argv[]) {
    
    Mat image = imread("/Users/hyunseung/Desktop/레드벨벳-슬기-패션.jpg.webp", 0);
    Mat image2 = imread("/Users/hyunseung/학부/4-1/영상처리/MedianFilterInput.png", 0);
    Mat MedianFilterInput = imread("/Users/hyunseung/학부/4-1/영상처리/MedianFilterInput.png", 0);
